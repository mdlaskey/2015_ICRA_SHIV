...
\section{Experiments}
\fpnote{Michael, please also compare my comments/changes git version 273ecde - all these changes got overwritten when merging}
\subsection{Driving Example}
%A common benchmark in Reinforcement Learning  is a car driving around a track avoiding other cars
%\cite{argall2009survey}. To illustrate our methods ability in high dimensions, the car must follow a polygonal track
%using only HOG features extracted from images. If the car falls off the track, it is placed back on the center facing
%the angle of the polygonal track piece. We sample the polygonal track by drawing points from a Gaussian with a mean that
%is the centered of the video game work space. The convex hull of the sampled points is then computed. The car's control
%space $\mathcal{U} = \lbrace -15^\circ, 0, 15^\circ \rbrace$ or 3 discrete commands that change the angle of the car. The internal state space of the car is xy-coordinates and the angle it is facing. Our supervisor is a solver that uses state space search through the driving simulator to plan the next control.

\fpnote{Previous commit:
...
[what about $r_0$?] We compare the policy's performance in terms of minimization of an underlying cost function $c(\bx,\bu)$, which is
