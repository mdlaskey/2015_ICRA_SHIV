...
\begin{abstract}
In traditional Imitation Learning, a human expert provides a few demonstrations to the robot, which then extrapolates these examples to new situations. One challenge is that during execution, any small but inevitable error will amplify, taking the robot further and further away from the regions of state space for which it has seen examples. Iterative imitation learning algorithms such as DAgger \cite{}\adnote{cite} address this by iteratively executing a trained policy, and asking the expert for examples along the visited states, which are then aggregated with the previous training data. However, such algorithms rely much more heavily on extensive human supervision.

Our goal is to reduce the amount of human supervision required in imitation learning, without harming performance, thus bridging traditional and iterative imitation learning. \emph{Our key insight is that the robot should ask for supervision only when its current trained policy has high risk.}

We introduce a method for iterative imitation learning that evaluates risk based on novelty of a state, as well as prior difficulty in labeling a nearby states. Our experiments with a toy car domain, as well as with non-prehensile grasping in clutter, suggest that our method can better utilize a human supervision budget, achieving higher succhess rates for the same number of queries to the expert.
