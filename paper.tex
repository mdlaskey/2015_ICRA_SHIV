...
\begin{abstract}
Algorithms such as DAgger for robot learning from demonstrations can be very effective for problems where system dynamics and reward functions are unknown, but can impose a burden on supervisors to provide labels (control signals) as states are encountered during policy rollouts.   To reduce this burden we introduce a new query selection method for stream-based active learning. The SHIV algorithm (Svm-based reduction in Human InterVention) only requests supervision for risky stater.  To facilitate scaling and outlier rejection, risk is based on distance to an approximate level set boundary defined by a One Class support vector machine.  We compare SHIV with DAgger in three policy learning experiments: 1) a driving simulator, 2) multi-joint robot arm grasping in clutter simulation with Box2D, and 3) surgical needle insertion.  Results suggest that SHIV can learn policies and reduce operator burden significantly  (by a factor of 3x to 5x).
...
\begin{tabular}{ R{4cm}||R{1.65cm}|R{1.65cm}|R{1.65cm}  }
 ...
 %\hline 
 & \multicolumn{1}{c}{Mis-Prediction Error} \\
 \hline
