...
In Fig. \ref{fig:car_cost}, we visualize the performance of DAgger and SHIV.  We run 6 \fpnote{do we
have large numbers of iterations in the final version, or is this the final one?} iterations \fpnote{define "iteration"
how many simulation cycles/size of $T$?} of each algorithm over 40 different
randomized polygonal tracks \fpnote{describe how you generated the tracks}. Figure \ref{fig:car_cost} presents averaged
results from these experiments, suggesting a $70\%$ reduction in the number of queries needed for SHIV compared to DAgger. 
\fpnote{don't use "initial results" when talking about your main experiment - makes it sound too weak}
 
\textbf{Comparison to active learning approaches:}
\fpnote{Refer to figure 3 here and explain, also explain "prediction error" do you mean mean total surrogate loss,
explain that you are using the scaled confidence intervals}
...
$0.1$ cm. The distance between initial $P_0$ and $P_G$ \fpnote{earlier when you introduce $P_0$, $P_G$ describe how many
and introduce a set- just 3 pairs, one per trajectory?} is roughly $3$ cm. Our cost function $c(\bx,\bu)$ is measured in
terms of Euclidean distance in translational component, which is in centimeters. \fpnote{no notion of cost on
rotations??} We run DAgger and SHIV both for 20 iterations and average over 40 rounds \fpnote{what's a 'round'?}.
Initial results \fpnote{they are not initial if you submit them...} shown in Fig. \ref{figures/needle_insertion_results.eps} suggest a reduction $67\%$ in the amount of human inquiries needed. 
