...
\caption{An illustrative example of different active learning techniques. The shaded and empty circles represent different labeled classes $0$ and $1$. The red empty circles represent label $1$, but are encountered on a different distribution then what was trained on. As illustrated, Confidence and Query by Committee methods measure states far away from the decision boundary of the estimator as low risk, however novelty detection methods measure the level of risk as how far a point is from the trained data. Our modified risk method attempts to measure risk both as a function of how far it is from data trained on and the decision boundary of the estimator. }
...
nature\adnote{avoid filler words like in nature}. The environment is
modeled\adnote{avoid passive voice; sidd always asked me whenever i wrote that "X is done": "done by who? by jesus?"} to have dynamics that are stationary\adnote{why stationary? what does this mean?} and Markovian, such that the the probability of state $\mathbf{x_{t+1}}\in
\mathcal{X}$ can be determined from the previous state $\mathbf{x}_t\in\mathcal{X}$ and control input $\mathbf{u}_t\in \mathcal{U}$,
so that\adnote{this sentence already contained a so that; break long sentences} $p(\bx_{N+1}|\bu_{N},\bx_{N}, \ldots, \bu_{0}, \bx_{0})=p(\bx_{N+1}|\bu_{N}, \bx_N)$.  
When no confusion arises\adnote{what is confusion arising}, we denote the probability density over the initial state also by $p:\mathcal{X}\to
\mathbb{R}$. By a trajectory, we mean \adnote{new par, a trajectory is a finite ...}a finite series of pairs of $T$ states visited and corresponding
...
At times we will also consider trajectories in state space only\adnote{this makes you seem floppy; do we need this? if so maybe denot by something else, like $\bar{tau}$ and call it a full trajectory vs a state trajectory}, in which case a trajectory corresponds
to a sequence of states $\tau = (\bx_0,....,\bx_T)$ with $\bx_i\in\mathcal{X}$ for $i\in \{0, \ldots, T\}$.


By a policy, we mean\adnote{a policy is a function} a function $\pi: \mathcal{X} \to \mathcal{U}$ from states to control inputs. 
We shall\adnote{we search in a space of parametrized policies} in particular consider classes of policies $\pi_{\theta}:\mathcal{X}\to \mathcal{U}$ parameterized by a
parameter $\theta\in \mathbb{R}^d$ corresponding to support vectors of a Support Vector Machine (SVM).\adnote{this is not a requirement that it's the svm necessarily, can be whatever estimator}
Any such policy, $\pi_{\theta}$ \adnote{missing comma here?} in an environment with probabilistic initial state density and Markovian dynamics
...
\adnote{Let} $p(\bx_t|\theta)$ denote the value of the density of states visited at time $t$ if the robot follows the policy
$\pi_{\theta}$ for $t-1$ time steps\adnote{break sentence. This can be computed by marginalizing ... (explain in words first)} which can be calculated by marginalization $p(\bx_t|\theta) =
...
\bx)$ or $p(\bx|\theta)$, but\adnote{while.., but.. doesn't make sense; "We do not assume... Instead, we assume that..."} instead assume that we have a stochastic real robot or a simulator such that for any state
$\bx_t$ and control $\bu_t$, we can sample the $\bx_{t+1}$ such that\adnote{redundant; we can sample b from $p(b......$"} $\bx_{t+1}$ follows the distribution corresponding
to the density $p(\bx_{t+1}|\pi_{\theta}(\bx_t),\bx_t)$. In particular\adnote{this is not really in particular; I think you should just say therefore, when rolling out..}, when 'rolling out' trajectories under a policy
$\pi_{\theta}$, we utilize the robot or simulator to 'sample' the resulting stochastic trajectories rather than
estimating $p(\bx|\theta)$ itself.

Typically, the objective in policy learning can be framed\adnote{can be framed by great justice; suggest: typically, the objective of policy learning is to find a policy that minimizes some known cost function:} as minimizing some cost function $C(\tau) = \sum^T_{t=1} c(\bx_t,\bu_t)$
...
be considered \cite{levine2015end}.  In our present setting\adnote{in lfd, the robot does not have access to the cost function itself. Instead, it only has access to a ...}, we do not know the cost function itself, but instead have access to a "supervisor", 
an algorithm or human\adnote{don't start the enumeration with algoirhtm and put human second in a paper motivated by reducing supervisory load, which is more important with human supervision} that we assume uses some near-optimal policy $\tilde{\pi}$ to minimize $C(\tau)$, and we are given
an initial set of $N$ stochastic demonstration trajectories $\mathcal{D} = \lbrace \tilde{\tau}^1,...,\tilde{\tau}^N \rbrace$. 
which are the result of the supervisor applying this policy. \adnote{sentences too long, break them up}
...
\todo{Getting a paper from Animesh to motivate this}
