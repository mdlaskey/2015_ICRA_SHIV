...
Rather than explicitly learning the cost function, like in Inverse Reinforcement Learning\adnote{cite}, and the dynamics model, and then using optimization to produce a policy for the task, in model-free LfD the robot learns the policy directly from supervisor examples mapping states to controls~\cite{argall2009survey}. 
...
\adnote{OK fiiiiiine. Then maybe To our knowledge, incorporating a query selection method for nonstationary distributions, such as the One Class SVM, into online LfD is a novel contribution.}
...
\noindent\textbf{Modeling Choices and Assumptions}  We model the system dynamics as Markovian, stochastic, and stationary. Stationary dynamics meaning\adnote{this sentence is missing a predicate; Stationary dynamics occur when, given..}  given a state and a control, the probability of the next state doesn't\adnote{does not} change over time. Note this is different then\adnote{from} the non-stationary distribution over the states the robot encounters during learning. \adnote{very good addition here!}

We model the initial state as sampled from a distribution over the state space.
We assume a known state space and set of controls. We also assume access to a robot or simulator, such that we  can sample from the state sequences induced by a sequence of controls.   Lastly, we assume access to a supervisor who can, given a state, provide a control signal label. We assume the supervisor can be noisy and imperfect, noting that the robot cannot surpass the level of performance of the supervisor. \adnote{not in general just in our work}

\adnote{again, this would be better at the same time with introducing the basic notation, like states and controls, so that you can state mathematically what these mean; for instance, you restate the markovian modelling anyway}
...
be considered \cite{levine2015end}.  \adnote{can be considered? like you'll allow it? maybe For example, in the task... , the cost function could be ...}

In our problem, we do not have access to the cost function itself. Instead, we only have access to 
a supervisor that can achieve a desire\adnote{d} level of performance on the task. \adnote{furthermore doesnt make sense here because the supervisor is providing these; say instead The supervisor provides the robot an initial set..} We are furthermore given
...
We define a `surrogate' loss function \adnote{as in \cite{ross2010reduction}, }$l:\mathcal{U}\times \mathcal{U}\to \mathbb{R}$, which provides a distance
...
\end{align}
with $p(\bx|\theta)$ from Eq. \ref{eq:density}.\adnote{fix}
...
\adnote{I don't like the flow of this coming before the algorithm. You can understand the algorithm without understanding the specifics of risk, but here I read this and still don't know how the algorithm is going to work and it's like keeping me in (bad) suspense until the next section. It also breaks flow because the last section set up the optimization problem that shiv and dagger are solving}
\begin{figure*}[t]
\centering
...
iteration can impose a large burden on the supervisor.\adnote{but you haven't described the algorithm yet so how am i supposed to know what that means; this is no longer intro land so stuff should be precise not handwavy; that is why this section doesnt fit here}
