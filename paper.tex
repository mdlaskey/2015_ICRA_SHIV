...
\caption{ \adnote{make this a 2 by 2 grid; a b c d, where d is the result; make the result graph more square}
...
used to control the smoothness of the function that is fit to the sampled data. In practice this regularization corresponds to a penalty term on either the L2 norm on the weights for regression based techniques or the slack coefficient for support vector machines \cite{scholkopf2002learning}.\adnote{is this just us or also dagger?}
 ...
 \subsection{Step 2}
The second step starts in boht SHIV and DAgger by rolling out the policy $\pi_{\theta_{k}}$ to sample states that are likely to occur under $\theta_{k}$. 
...
deduced error bounds on the expected loss, which scale linearly in the time horizon $T$ --- as opposed to offline methods which scale quadratically. For a conservative threshold $\rho$ on risk, the SHIV performance is analogous to DAgger without asking the expert when it is absolutely not necessary. However, if $\rho$ is too high, then SHIV will not collect new that that is needed and revert back to the performance of offline methods. We analyze the dependence on $\rho$ in our experimental section. Our results suggest that SHIV is robust to this parameter selection. \adnote{must add this experiment!}
